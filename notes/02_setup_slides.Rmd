...
Note: We're cheating a little bit. The [Tidyverse](https://www.tidyverse.org/) is "an opinionated collection of R packages designed for data science. All packages share an underlying design philosophy, grammer, and data structures". In other words, they are designed to play really well together.

So, instead of separately installing `ggplot2`, `rmarkdown`, `dplyr`, `readxl` among others, we are choosing to install the complete tidyverse at one time. 
