...
`r emo::ji("arrow_forward")` In the console type the following code, then press `Enter`.
...
`r emo::ji("arrow_forward")` Put a open parenthesis `(` before `log10` to fix it and try again. 
...
`r emo::ji("arrow_forward")`  Start by typing the following in the console to install the `ggplot2` package. 
...
.pull-left[<br>
`r emo::ji("arrow_forward")` Type these commands into the console one at a time and wait for it to finish before entering the next command.
]
...
`r emo::ji("warning")` Don't let not knowing exactly how to describe your problem prevent you from asking. Screenshots help temendously
...
`r emo::ji("warning")` Don't spend more than 20 minutes banging your head on the wall before you ask the instructor for help!
...
Unfortunately in person `r emo::ji("busts_in_silhouette")` help not available during Fall 20 due to COVID19 `r emo::ji("mask")`
...
* Attend Community Coding.
  - Hours can be found here: [bit.ly/cc_hours](bit.ly/cc_hours)
